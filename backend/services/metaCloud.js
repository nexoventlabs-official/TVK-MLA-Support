 ...
 * it as an AUTHENTICATION-category template — Meta auto-generates the body
 * ("{{1}} is your verification code. For your security, do not share this
 * code. This code expires in N minutes.") which matches the temp.png
 * reference shot exactly, and the OTP/COPY_CODE button gives the user a
 * native one-tap copy experience.
 *
 * AUTHENTICATION templates auto-approve on creation, so the very first
 * cold-start send self-heals: 132001 → ensureOtpTemplate → instant approval
 * → retry succeeds in the same request.
 *
 * Returns `{ status, created }` so callers can react if the status is
 * unexpectedly PENDING (rare, usually means the WABA itself is gated).
 */
...
/**
 * Send the AUTHENTICATION-category template that delivers a 6-digit OTP.
 *
 * Used when the user is OUTSIDE the 24-hour window (or has never messaged
 * the bot). The template body is auto-generated by Meta — see the comment
 * on `ensureOtpTemplate` for the exact wording. The OTP COPY_CODE button
 * takes the code as a plain text parameter under sub_type 'url' (Meta's
 * convention for OTP buttons regardless of otp_type).
 *
