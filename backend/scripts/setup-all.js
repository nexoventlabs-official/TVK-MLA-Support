/**
 * Orchestrated WhatsApp Flow setup for TVK:
 *   1) Generate RSA keypair (or reuse existing)
 *   2) Write FLOW_PRIVATE_KEY / FLOW_PUBLIC_KEY into .env
 *   3) Upload public key to Meta
 *   4) Create + publish the flow with endpoint_uri = `${BACKEND_URL}/api/flow-endpoint`
 *   5) Write WHATSAPP_FLOW_ID / WHATSAPP_FLOW_STATUS into .env
 *
 * Requires `BACKEND_URL` to already be an HTTPS URL (e.g. ngrok).
 *
...
(async () => {
  ...
  let flowId;
  try {
    const r = await meta.createFlow('TVK Grievance', ['OTHER'], { endpointUri });
    flowId = r.id;
